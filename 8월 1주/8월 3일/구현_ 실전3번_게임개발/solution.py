...
# 0 -> 북, 1 -> 동, 2 -> 남, 3 -> 서
# 왼쪽방향 부터 탐색
# 왼쪽 가보지 않는 칸이 존재 한다면, 회전 후 전진
# 왼쪽 가보지 않는 칸이 존재 안하면, 회전만
# 4방향 모두 가봤거나 바다로 되어 있으면, 바라보는 방향을 유지 한뒤 한칸 뒤로가고 다시 탐색한다
# 뒤쪽 방향이 바다 인경우 움직임을 멈춘다
# 캐릭터가 방문한 지역의 개수를 Return

# 1. 순서대로 탐색 해야하나?
# 2. 모두 가봤거나 바다로 되어 있으면 바라보는 방향(원래 위치) n == 4 인 경우
# 3. 회전 해서 탐색 => moves (moves[(i + 1) % 4], (i + 1) % 4)
# 4. 모두 탐색 했으면 => back (moves[(i + 2) % 4] 로 설계, 바라 보는 방향은 유지)
# 5. back 했을 때 없으면 종료
...
def solution():
    ...
    while True:
        ...
        # 한바퀴 돌기
        for i in range(4):
            d_direction = (direction + (3 - i)) % 4

            d_row, d_col = moves[d_direction]
            n_row, n_col = row + d_row, col + d_col
            if maps[n_row][n_col] == 0 and (n_row, n_col) not in visited:
                move = True
        ...
        # 뒤로가기
        if not move:
            d_row, d_col = moves[(direction + 2) % 4]
            n_row, n_col = row + d_row, col + d_col

            if maps[n_row][n_col] == 1:
                break
